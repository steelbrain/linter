...
export default class EditorLinter {
  constructor(editor) {
    ...
    this.subscriptions.add(this.onDidMessageAdd(message => {
      ...
      if (this.gutterEnabled) {
        //TODO: Beware, hacks ahead due to atom/atom#8920
        const rowClass = 'linter-' + message.range.start.row
        this.gutter.decorateMarker(marker, {
          class: rowClass,
          onlyHead: true
        })
        requestAnimationFrame(() => {
          let el = this.gutterView.getElementsByClassName(rowClass)
          el = el[el.length - 1]
          el.classList.add('linter-highlight')
          el.classList.add('decoration')
          el.classList.add(message.class)
          el._classList = el.classList
  ...
  addGutter() {
    ...
    this.gutterView = atom.views.getView(this.gutter)
  }
