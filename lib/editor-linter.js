...
export default class EditorLinter {
  ...
  constructor(editor: TextEditor) {
    ...
    this.subscriptions.add(this.editor.onDidSave(
      debounce(() => this.emitter.emit('should-lint', false), 500, true),
    ))
    this.subscriptions.add(this.editor.getBuffer().onDidReload(
      debounce(() => this.emitter.emit('should-lint', false), 1000, false),
    ))
