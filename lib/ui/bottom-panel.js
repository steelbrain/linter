...
export default class BottomPanel {
  constructor(scope, editorRegistry) {
    ...
    this.subscriptions.add(editorRegistry.observe(editorLinter => {
      ...
      editorLinter.onDidDestroy(() => {
        container.remove()
        editorLinter.getMessages().forEach(message => {
          if (message.scope === 'file') {
            this.__removeMessage(message)
          }
        })
      ...
      editorLinter.onDidMessageChange(({message, type}) => {
        ...
        } else this.__removeMessage(message)
      })
    }))
  ...
  refresh(scope) {
    ...
    if (scope === 'File') {
      ...
      this.editorMessages.forEach((messagesElement, editorLinter) => {
        if (editorLinter === activeEditorLinter) {
