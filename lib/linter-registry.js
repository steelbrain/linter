...
export default class LinterRegistry {
  ...
  lint({onChange, editorLinter}) {
    ...
    if (
      ...
      Helpers.isPathIgnored(editor.getPath(), this.ignoreGlob)     // Ignored by VCS or Glob
    ) {
      return
    }
    ...
    this.linters.forEach(linter => {
      if (Helpers.shouldTriggerLinter(linter, onChange, scopes)) {
        this.emitter.emit('did-begin-linting', {linter, filePath})
        promises.push(new Promise(function(resolve) {
          resolve(linter.lint(editor))
        }).then(results => {
          this.emitter.emit('did-finish-linting', {linter, filePath: linter.scope === 'project' ? null : filePath})
          if (results) {
