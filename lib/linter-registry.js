...
export default class LinterRegistry {
  ...
  async lint({ onChange, editor } : { onChange: boolean, editor: TextEditor }): Promise<boolean> {
    ...
    for (const linter of this.linters) {
      ...
      this.emitter.emit('did-begin-linting', { number, linter, filePath: linter.scope === 'file' ? filePath : null })
      promises.push(new Promise(function(resolve) {
        resolve(linter.lint(editor))
      }).then(messages => {
        this.emitter.emit('did-finish-linting', { number, linter, filePath: linter.scope === 'file' ? filePath : null })
        if (Array.isArray(messages) && linter[$requestLastReceived] < number && linter[$activated]) {
          ...
          if (!buffer || buffer.isAlive()) {
            const validity = atom.inDevMode() ? (linter[$version] === 2 ? validateMessages(linter.name, messages) : validateMessagesLegacy(linter.name, messages)) : true
            if (validity) {
              if (linter[$version] === 2) {
