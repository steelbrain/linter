...
import { $version, $activated, $requestLatest, $requestLastReceived } from './constants'
import type { Linter, State } from './types'
...
class LinterRegistry {
  ...
  async lint({ onChange, editor } : { onChange: boolean, editor: TextEditor }, projectLinters: boolean = false): Promise<boolean> {
    const filePath = editor.getPath()

    if (
      (onChange && !this.lintOnChange) ||                                                       // Lint-on-change mismatch
      !filePath ||                                                                              // Not saved anywhere yet
      Helpers.isPathIgnored(editor.getPath(), this.ignoreVCS) ||                                // Ignored by VCS or Glob
    ...
    for (const linter of this.linters) {
      ...
      if (projectLinters && (linter.scope !== 'file' || !linter.lintsProject)) {
        continue
      }
      if (this.disabledProviders.includes(linter.name)) {
        continue
      }
      const statusName = linter[$version] === 2 ? linter.name : null
      const statusBuffer = linter.scope === 'file' ? editor.getBuffer() : null
      const statusFilePath = linter.scope === 'file' ? filePath : null
      if (linter[$version] === 2 && projectLinters && statusName && statusFilePath) {
        const invocationsKey = `${statusName}$${statusFilePath}`
        if (!this.invocations.has(invocationsKey)) {
          this.invocations.add(invocationsKey)
          const messages: ?Array<Object> = Helpers.findFromStateMessages(this.state.messages, statusName, statusFilePath)
          if (messages) {
            Helpers.normalizeMessages(statusName, messages, '$SERIALIZED')
            this.emitter.emit('did-update-messages', {
              messages,
              linter: null,
              linterName: statusName,
              editorPath: statusFilePath,
            })
            return true
          }
        }
      ...
      promises.push(new Promise(function(resolve) {
        ...
        this.emitter.emit('did-update-messages', {
          messages,
          linter: linter[$version] === 1 ? linter : null,
          linterName: statusName,
          editorPath: statusFilePath,
        })
      }, (error) => {
