...
class LinterRegistry {
  ...
  async lint({ onChange, editor, scope } : { onChange: boolean, editor: TextEditor, scope: ?string }): Promise<boolean> {
    ...
    for (const linter of this.linters) {
      if (!Helpers.shouldTriggerLinter(linter, onChange, scopes, scope)) {
        continue
      }
