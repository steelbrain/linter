...
class LinterRegistry {
  ...
  async lint({ onChange, editor } : { onChange: boolean, editor: TextEditor }, projectLinters: boolean = false): Promise<boolean> {
    ...
    const invocationKey = `${filePath}$${onChange}`
    try {
      if (projectLinters && this.state.messages && !this.invocations.has(invocationKey)) {
        const oldMessages = this.state.messages.filter(entry => (entry.editorPath === filePath))
      ...
      this.invocations.add(invocationKey)
    }
    ...
    for (const linter of this.linters) {
      ...
      const linterName = linter[$version] === 2 ? linter.name : null
      ...
      promises.push(new Promise(function(resolve) {
        ...
        this.emitter.emit('did-update-messages', { messages, linter, linterName, editorPath: statusFilePath })
      }, (error) => {
        this.emitter.emit('did-finish-linting', { number, linter, filePath: statusFilePath })
