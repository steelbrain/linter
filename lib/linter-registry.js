...
import { $version, $activated, $requestLatest, $requestLastReceived, LINTER_CONFIG_FILE_PATH, LINTER_CONFIG_FILE_DEFAULT, LINTER_CONFIG_FILE_OPTIONS } from './helpers'
...
export default class LinterRegistry {
  ...
  async getConfig(): Promise<ConfigFile> {
    if (!this.config) {
      this.config = await ConfigFile.get(LINTER_CONFIG_FILE_PATH, LINTER_CONFIG_FILE_DEFAULT, LINTER_CONFIG_FILE_OPTIONS)
    }
    return this.config
  }
