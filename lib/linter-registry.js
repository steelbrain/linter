...
import { $version, $activated, $requestLatest, $requestLastReceived, LINTER_CONFIG_FILE_PATH, LINTER_CONFIG_FILE_DEFAULT } from './helpers'
...
export default class LinterRegistry {
  ...
  async lint({ onChange, editor } : { onChange: boolean, editor: TextEditor }): Promise<boolean> {
    ...
    for (const linter of this.linters) {
      ...
      if (disabled.indexOf(linter.name) !== -1) {
        continue
      }
