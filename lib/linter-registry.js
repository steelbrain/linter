        ...
        new Promise(function(resolve) {
          ...
          messages => {
            ...
            if (atom.inDevMode() || !Array.isArray(messages)) {
              validity =
                linter[$version] === 2
                  ? // $FlowIgnore: Temporarily suppressed till we remove support for legacy protocol
                    Validate.messages(linter.name, messages)
                  : // $FlowIgnore: Temporarily suppressed till we remove support for legacy protocol
                    Validate.messagesLegacy(linter.name, messages)
            }
            if (!validity) {
              return
            }
