...
import { getEditorCursorScopes, normalizeMessages, normalizeMessagesLegacy, isPathIgnored, shouldTriggerLinter, $version, $activated, $requestLatest, $requestLastReceived } from './helpers'
...
export default class LinterRegistry {
  ...
  addLinter(linter: Linter, legacy: boolean = false) {
    ...
    if (!validateLinter(linter, version)) {
      return
    }
  ...
  async lint({ onChange, editor } : { onChange: boolean, editor: TextEditor }): Promise<boolean> {
    ...
    const scopes = getEditorCursorScopes(editor)
