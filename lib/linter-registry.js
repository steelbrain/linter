...
import { getEditorScopes, normalizeMessages, normalizeMessagesLegacy, isPathIgnored, shouldTriggerLinter, $version, $activated, $requestLatest, $requestLastReceived } from './helpers'
...
export default class LinterRegistry {
  ...
  async lint({ onChange, editor } : { onChange: boolean, editor: TextEditor }): Promise<boolean> {
    ...
    const scopes = getEditorScopes(editor)
