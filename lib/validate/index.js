...
function validateMessages(linterName: string, entries: Array<Message>): boolean {
  ...
  if (Array.isArray(entries)) {
    ...
    for (let i = 0, length = entries.length; i < length; ++i) {
      ...
      if (!invalidSource && source && (typeof source !== 'object' || typeof source.file !== 'string' || typeof source.position !== 'object' || !source.position)) {
        invalidSource = true
        messages.push('Message.source must be valid')
      } else if (!invalidSource && source) {
        const position = Point.fromObject(source.position)
        if (Number.isNaN(position.row) || Number.isNaN(position.column)) {
          invalidSource = true
          messages.push('Message.source.position should not contain NaN coordinates')
        }
      }
      ...
      if (!invalidReference && message.reference && typeof message.reference !== 'string') {
        ...
        messages.push('Message.reference must a string')
      }
