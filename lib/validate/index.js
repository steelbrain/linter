...
import type { UI, Linter, Message, MessageOld, IndieConfig } from '../types'
...
function validateMessagesLegacy(linterName: string, entries: Array<MessageOld>): boolean {
  const messages = []
  ...
  if (Array.isArray(entries)) {
    ...
    for (let i = 0, length = entries.length; i < length; ++i) {
      ...
      if (!invalidContent && ((typeof message.text !== 'string' && (message.html !== 'string' && !(message.html instanceof HTMLElement))) || (!message.text && !message.html))) {
        invalidContent = true
      ...
      if (!invalidFix && message.fix && (typeof message.fix.range !== 'object' || message.fix.newText !== 'string' || (message.fix.oldText && message.fix.oldText !== 'string'))) {
        invalidFix = true
