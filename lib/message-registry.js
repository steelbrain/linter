...
import type { MessagesPatch, Message, MessageLegacy, Linter, Linter$Message$Map, StateMessages } from './types'
...
class MessageRegistry {
  ...
  set({ messages, linter, linterName, editorPath }: { messages: Array<Message | MessageLegacy>, linter: ?Linter, linterName: ?string, editorPath: ?string }) {
    ...
    for (const entry of this.messagesMap) {
      if (
        ...
        (
          (linterName && entry.linterName === linterName) ||
          (linter && entry.linter === linter)
        )
      ) {
        found = entry
        break
    ...
    if (found) {
      ...
      this.messagesMap.add({
        version: linterName ? 2 : linter[$version],
        linter: linterName ? null : linter,
        linterName,
        editorPath,
  ...
  serialize(): Array<StateMessages> {
    const messages = []
    for (const entry: Linter$Message$Map of this.messagesMap) {
      if (entry.version === 2 && !entry.deleted && entry.messages.length && typeof entry.editorPath === 'string') {
        messages.push({
          serializedVersion: 1,
