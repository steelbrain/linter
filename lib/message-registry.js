...
import { $version, trimMessages, messageKey, messageKeyLegacy } from './helpers'
import type { MessagesPatch, Message, MessageLegacy, Linter, Linter$Message$Map, State } from './types'
...
class MessageRegistry {
  ...
  set({ messages, linter, buffer }: { messages: Array<Message | MessageLegacy>, linter: Linter, buffer: ?TextBuffer }) {
    ...
    if (found) {
      ...
      this.messagesMap.add({
        ...
        linter: linter[$version] === 2 ? null : linter,
  ...
  // NOTE: Only serializes messages for Standard and Indie providers v2
  serialize(): Array<Linter$Message$Map> {
    const messages: Array<Linter$Message$Map> = []
    for (const entry: Linter$Message$Map of this.messagesMap) {
      if (entry.version !== 2 || entry.deleted || !entry.messages.length) continue
      messages.push({
        version: 2,
        linter: null,
