...
import ProjectsRegistry from './project-registry'
import * as decorators from './decorators'

import type UIRegistry from './ui-registry'
import type IndieRegistry from './indie-registry'
import type LinterRegistry from './linter-registry'
import type MessageRegistry from './message-registry'
import type EditorsRegistry from './editor-registry'
...
class Linter {
  ...
  registryProjects: ProjectsRegistry;
  ...
  constructor(state: State) {
    ...
    this.registryProjects = new ProjectsRegistry(state)
    this.subscriptions.add(this.commands)
    this.subscriptions.add(this.registryProjects)

    this.registryProjects.onShouldLint((options) => {
      this.registryLintersInit()
      options.waitHandle = this.registryLinters.lint({ onChange: false, editor: options.editor }, true)
    })
  ...
  @decorators.lazyRequire('./editor-registry', 'registryEditors')
  registryEditorsInit() {
    this.registryEditors.observe((editorLinter) => {
      ...
      editorLinter.onDidDestroy(() => {
        this.registryMessagesInit()
        this.registryMessages.deleteByEditorPath(editorLinter.getEditor().getPath())
      })
    })
    this.registryEditors.activate()
  }
  @decorators.lazyRequire('./linter-registry', 'registryLinters')
  registryLintersInit() {
    this.registryLinters.onDidUpdateMessages(({ linter, linterName, messages, editorPath }) => {
      this.registryMessagesInit()
      this.registryMessages.set({ linter, linterName, messages, editorPath })
    })
  ...
  @decorators.lazyRequire('./indie-registry', 'registryIndie')
  registryIndieInit() {
    ...
    this.registryIndie.onDidUpdate(({ linter, linterName, messages }) => {
      this.registryMessagesInit()
      this.registryMessages.set({ linter, linterName, messages, editorPath: null })
    })
  }
  @decorators.lazyRequire('./message-registry', 'registryMessages')
  ...
  @decorators.lazyRequire('./ui-registry', 'registryUI')
  registryUIInit() {}
