...
export default class Linter {
  ...
  constructor() {
    ...
    this.commands.onShouldDebug(async () => {
      ...
      const manifest = atom.packages.getLoadedPackage('linter').metadata
    ...
    this.commands.onShouldToggleLinter((action) => {
      ...
      const linterNames = arrayUnique(this.registryLinters.getLinters().map(linter => linter.name))

      const toggleView = new ToggleView(action, linterNames)
    ...
    const mainCallbackId = window.requestIdleCallback(function linterMainIdleCallback() {
