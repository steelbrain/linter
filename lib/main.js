...
class Linter {
  ...
  constructor(state: Object) {
    this.state = Helpers.fillPackageState(state)
    ...
    this.commands.onShouldShowLinterProviders(() => {
      const linters = this.registryLinters.getLinters()
      const textEditor = atom.workspace.getActiveTextEditor()
      const textEditorScopes = textEditor.scopeDescriptorForBufferPosition(textEditor.getCursorBufferPosition()).getScopesArray().concat(['*'])
      const matchingLinters = linters.filter(linter => Helpers.shouldTriggerLinter(linter, false, textEditorScopes)).sort((a, b) => a.name.localeCompare(b.name))
      atom.notifications.addInfo('Matching Linter Providers', { description: matchingLinters.map(i => `  - ${i.name}`).join('\n'), dismissable: true })
    })
