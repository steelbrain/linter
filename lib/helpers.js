...
import type { State, Linter, Message, MessageLegacy } from './types'
...
export const LINTER_CONFIG_FILE_PATH = Path.join(atom.getConfigDirPath(), 'linter-config.json')
export const LINTER_CONFIG_FILE_DEFAULT = {
  disabled: [],
}
export const LINTER_CONFIG_FILE_OPTIONS = {
  prettyPrint: true,
  createIfNonExistent: false,
}
export function getConfigFile(): Promise<ConfigFile> {
  return ConfigFile.get(LINTER_CONFIG_FILE_PATH, LINTER_CONFIG_FILE_DEFAULT, LINTER_CONFIG_FILE_OPTIONS)
}
export function fillPackageState(given: Object): State {
  const state = {}

  state.version = given.version || 2
