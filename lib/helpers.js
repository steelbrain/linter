...
export async function shouldTriggerProjectLinter(linter: Linter, filePath: string): Promise<boolean> {
  const fileisIgnored = !linter.isFileIgnored || await linter.isFileIgnored(filePath)

  return !!(linter.scope === 'file' && linter.lintsProject && fileisIgnored)
}
