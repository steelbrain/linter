...
import type { Linter, Message, MessageLegacy, StateMessages } from './types'
...
export function shouldTriggerLinter(
  ...
  return scopes.some(item => linter.grammarScopes.includes(item))
}
...
export function isPathIgnored(filePath: string, ignoredVCS: boolean): boolean {
  if (ignoredVCS) {
    let repository = null
...
export function normalizeMessages(linterName: string, messages: Array<Message>, keyPrefix: ?string = null) {
  for (let i = 0, length = messages.length; i < length; ++i) {
    ...
    message.key = `${keyPrefix || ''}${messageKey(message)}`
  }
}
...
export function trimMessages(messages: Array<Object>): Array<Object> {
  return messages.map((message: Message) => ({
    version: 2,
    linterName: message.linterName,
...
export function findFromStateMessages(messages: ?Array<StateMessages>, linterName: string, editorPath: string): ?Array<Message | MessageLegacy> {
  let found = null
  if (messages) {
    found = messages.find(entry => (entry.linterName === linterName && entry.editorPath === editorPath))
  }
