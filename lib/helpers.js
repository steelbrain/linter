...
export function messageKey(message) {
  return (message.text || message.html) + '$' + message.type + '$' + message.class + '$' + message.name + '$' + message.filePath + '$' + (
    message.range ? (
      message.range.constructor.name === 'Array' ?
      message.range[0][1] + ':' + message.range[0][0] + ':' + message.range[1][1] + ':' + message.range[1][0]:
      message.range.start.column + ':' + message.range.start.row + ':' + message.range.end.column + ':' + message.range.end.row
    ) : ''
  )
}
