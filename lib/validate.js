...
import type { UI, Linter, Message } from './types'

const VALID_SEVERITY = new Set(['error', 'warning', 'info'])

export function ui(provider: UI): boolean {
  let message
...
export function linter(provider: Linter, indie: boolean = false): boolean {
  let message
...
export function messages(entries: Array<Message>): boolean {
