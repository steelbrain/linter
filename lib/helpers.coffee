...
Helpers = module.exports =
  ...
  shouldTriggerLinter: (linter, onChange, scopes) ->
    # Trigger lint-as-you-type linters on both events but on-save linters only on save
    # Because we want to trigger lintAsYouType linters on save when the
    # user has disabled lintAsYouType from config
    return false if onChange and not linter.lintAsYouType
