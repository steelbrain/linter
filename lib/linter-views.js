...
export default class LinterViews {
  ...
  notifyEditorLinters({added, removed}) {
    const editorsMap = this.editors.getMap()
    removed.forEach(message => {
      if (message.filePath && typeof editorsMap[message.filePath] !== 'undefined') {
        editorsMap[message.filePath].forEach(function(editorLinter) {
    ...
    if (activeEditorLinter) {
      activeEditorLinter.calculateLineMessages(null)
      this.renderBubble(activeEditorLinter)
    }
  }
